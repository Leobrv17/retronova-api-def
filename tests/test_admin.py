...
class TestAdmin:
    ...
    def test_list_deleted_users_with_data(self, client, auth_headers_admin, sample_user, db):
        ...
        sample_user.deleted_at = db.execute("SELECT NOW()").scalar()
        db.commit()
    ...
    def test_admin_endpoints_unauthorized_no_auth(self, client, sample_arcade, sample_game):
        """Test d'accès aux endpoints admin sans authentification."""
        endpoints_to_test = [
            ("POST", "/api/v1/admin/arcades/",
             {"nom": "Test", "description": "Test", "localisation": "Test", "latitude": 0, "longitude": 0}),
            ("POST", "/api/v1/admin/games/", {"nom": "Test", "description": "Test"}),
