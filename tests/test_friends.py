...
class TestFriends:
    ...
    def test_get_friend_requests_with_pending_request(self, client, auth_headers_user, sample_user, friend_user, db):
        ...
        # Assure-toi que les utilisateurs sont bien persistés
        db.add_all([sample_user, friend_user])
        db.commit()

        # Recharge-les pour qu'ils soient bien attachés à la session
        ...
        # Sauvegarde l’ID pendant que la session est ouverte
        ...
        data = response.json()
        assert len(data) == 1
        assert data[0]["requester"]["id"] == friend_user_id
    ...
    def test_get_friends_after_acceptance(self, client, auth_headers_user, sample_user, friend_user, db):
        ...
        # Ajout explicite à la session pour être sûr que tout est en base
        db.add_all([sample_user, friend_user])
        db.commit()

        # Rafraîchir pour récupérer les IDs et éviter les objets détachés
        db.refresh(sample_user)
        db.refresh(friend_user)

        friend_user_id = friend_user.id  # Sauvegarde l’ID tant que l'objet est valide

        # Création du lien d'amitié accepté
        friendship = Friendship(
        ...
        assert len(data) == 1
        assert data[0]["id"] == friend_user_id
