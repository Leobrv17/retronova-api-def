...
class TestArcades:
    ...
    def test_get_arcade_with_games(self, client, sample_arcade, sample_game, db):
        ...
        assert data["games"][0]["id"] == sample_id
    ...
    def test_get_arcade_config_with_api_key(self, client, sample_arcade, sample_game, arcade_api_headers, db):
        ...
        expected_game_id = sample_game.id
        expected_arcade_id = sample_arcade.id
        expected_arcade_name = sample_arcade.nom

        response = client.get(f"/api/v1/arcades/{expected_arcade_id}/config", headers=arcade_api_headers)

        assert response.status_code == 200
        data = response.json()
        assert data["arcade_id"] == expected_arcade_id
        assert data["arcade_name"] == expected_arcade_name
        assert "games" in data
        assert len(data["games"]) == 1
        assert data["games"][0]["slot"] == 1
        assert data["games"][0]["game_id"] == expected_game_id
    ...
    def test_arcade_games_multiple_slots(self, client, sample_arcade,sample_game, db):
        ...
        game_ids = [sample_game.id, game2.id]
        ...
        assert 1 in slots
        assert 2 in slots
        assert slots[1]["id"] == game_ids[0]
        assert slots[2]["id"] == game_ids[1]
